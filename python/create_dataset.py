...
class RunMD:
    ...
    def run(self):
        ...
        self.lmp.command(f'thermo_style custom step pe')
        self.lmp.command(f'variable sstep equal step')
        self.lmp.command(f'variable ppt equal pe')
        self.lmp.command(f'fix extra all print {self.dump_freq} \"${{sstep}} ${{ppt}}\" file {self.save_dir}/traj_{self.idx_traj}/etotal.txt screen no')
